        ...
        public static void Initialize(IServiceProvider serviceProvider)
        {
            using (var context = new CommerceRazorDemoContext(serviceProvider.GetRequiredService<DbContextOptions<CommerceRazorDemoContext>>()))
            {
                ...
                if(!context.Product.Any())
                {
                    ...
                    {
                        new Product{ Title="Air Jordan Sneaker", Brand="Nike", Description="Original Air Jordan", Price=119.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Sports").Id},
                        new Product{ Title="Predator", Brand="Adidas", Description="Soccer shoe", Price=79.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Sports").Id },
                        new Product{ Title="Tiger Woods Shirt", Brand="Nike", Description="Tiger Woods final round shirt", Price=49.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Sports").Id },
                        new Product{ Title="Workout Top", Brand="Under Armour", Description="Basic workout shirt", Price=39.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Sports").Id },
                        new Product{ Title="Workout Short", Brand="Under Armour", Description="Basic workout short", Price=39.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Sports").Id },
                        new Product{ Title="Attack Helmet", Brand="Giro", Description="Giro Attack aerodymic cycling helmet", Price=99.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Sports").Id },
                        

                        new Product{ Title="Rivet Aiden Mid-Century Modern Tufted Velvet Loveseat Sofa", Brand="Rivet", Description="This sleek, mid-Century inspired loveseat is designed to impress. ", Price=739.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Household").Id },
                        new Product{ Title="HERCULES Lesley Series Contemporary Black LeatherSoft Sofa with Encasing Frame", Brand="Flash Furniture", Description="You're great at what you do and your company has grown so make sure you convey that to clients when you choose your reception furniture.", Price=1257.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Household").Id },
                        new Product{ Title="Premium Microfiber Cleaning Cloth", Brand="OVWO", Description="80% Polyester/ 20% Polyamide", Price=9.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Household").Id },
                        new Product{ Title="Bathroom Cleaner Spray", Brand="Lysol", Description="KILLS 99.9% OF BATHROOM VIRUSES AND BACTERIA", Price=3.97M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Household").Id },
                        new Product{ Title="Linen White Bath Towels 4-Pack", Brand="Hammam", Description=" 4-Piece super soft and absorbent Turkish cotton bath towels.", Price=33.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Household").Id },
                        new Product{ Title="Sure-Crisp Air Fryer Countertop Toaster Oven", Brand="Hamilton Beach", Description="The Sure-Crisp air fry convection function circulates air around food as it cooks", Price=69.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Household").Id },


                        new Product{ Title="Inspiron 3511 Premium Laptop", Brand="Dell", Description="Intel Core i5-1035G1 Quad-Core Processor (4Cores, 6MB Intel Smart Cache, 8 Threads", Price=679.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Technology").Id },
                        new Product{ Title="XPS 17 9720 Laptop", Brand="Dell", Description="12th Generation Intel Core i7-12700H (24MB Cache, up to 4.7 GHz, 14 cores)", Price=2499.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Technology").Id },
                        new Product{ Title="iPhone 13 Pro", Brand="Apple", Description="6.1-inch Super Retina XDR display with ProMotion for a faster, more responsive feel", Price=999.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Technology").Id },
                        new Product{ Title="iPhone 13 Mini", Brand="Apple", Description="5.4-inch Super Retina XDR display", Price=729.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Technology").Id },
                        new Product{ Title="65-Inch Class UQ9000", Brand="LG", Description="Everything you need to bring your favorite content to life with the power of 4K and the extras you crave.", Price=576.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Technology").Id },
                        new Product{ Title="iPad Air", Brand="Apple", Description="10.9-inch, Wi-Fi + Cellular, 64GB", Price=749.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Technology").Id }, 


                        new Product{ Title="Mens Tilden Cap Oxford Shoe", Brand="Clarks ", Description="A classic captoe derby crafted from rich, full grain leather.", Price=59.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Fashion").Id },
                        new Product{ Title="Mens Cotrell Step Slip-On Loafer", Brand="Clarks", Description="Slip-on casual", Price=54.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Fashion").Id },
                        new Product{ Title="Mens Dress Shirt Regular Fit Poplin Solid", Brand="Van Heusen", Description="Fabric features enhanced wrinkle resistance for easy care at home.", Price=24.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Fashion").Id },
                        new Product{ Title="Mens Long Sleeve Dress Shirt", Brand="Alberto Danelli", Description="Our cotton blend dress shirt is the perfect fitting day to night closet must have. ", Price=29.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Fashion").Id },
                        new Product{ Title="Mens Classic Fit Easy Khaki", Brand="Dockers", Description="64% Cotton, 34% Polyester, 2% Elastane", Price=39.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Fashion").Id },
                        new Product{ Title="Mens American Chino Flat Front Straight Fit Pant", Brand="Izod", Description="100% Cotton", Price=31.99M, AvailableQty=10, ProductCategoryId=context.ProductCategory.Single(x => x.Title == "Fashion").Id }
                    };
